...
export class XMPReader {
    ...
    static getLastKeyFromPath(path) {
    ...
    static getKeyFromPath(path) {
        return path
            ...
            .map((p, i) => i === 0 ? this.lowercaseFirstLetter(p) : this.capitalizeFirstLetter(p))
            .join('');
    }
                        ...
                        if (text.trim() !== '') {
                            ...
                            switch (nodeName) {
                                ...
                                case 'stArea:h':
                                    ...
                                    break;
                                case 'xmp:Rating':
                                    value = parseInt(text, null);
                                    break;
                                case 'MicrosoftPhoto:Rating':
                                    value = Math.floor((parseInt(text, null) + 12) / 25) + 1;
                                    break;
                                default:
                                    value = text;
                            }
